...
router.post('/submitted', async (req, res) => {
    try {
        const { merchantId, amount, nonce, signedHash, data, transactionHashes, transactiontype, users, extradata } = req.body;
        ...
        const verifier = verifyTransaction(merchant.merchantKey, messagecheck, signedHash)

        const isValidSignature = verifier
        ...
        switch (transactiontype) {
            ...
            case 1:
                success = check2(transactionHashes)
            case 2:
...
router.get('/signtest', async (req, res) => {
    let data = createTransactionData("322f07bf-6a92-4541-abb4-5b0f8f157774", "10", "abc", "john")
    console.log(data)
